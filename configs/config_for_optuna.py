...
class Config:
    ...
    def __dataset_common_params(self):
        return {
            "JHT": {
                # "image_feature_arch": "vit-base-patch16-224",
                "image_dir": os.path.join(self.work_dir, "data/Processed/spectrogram"),
            }
        }

    def __model_config_params(self, trial):
    ...
    def get_config(self, trial):
        return Storage(
            dict(
                self.global_params,
                **self.__model_config_params(trial)["dataset_params"][
                    self.global_params["dataset_name"]],
                **self.__model_config_params(trial)["common_params"],
                **self.__dataset_common_params()[
                    self.global_params["dataset_name"]]))
