...
def calculate_metrics_for_classification(outputs, targets, num_classes):
    """
    计算模型的准确率、召回率、F1等指标
    :param outputs: 模型输出结果，为N * num_classes
    :param targets: 样本标签，为N * number_classes
    :param num_classes
    :return: 预测准确度
    """
...
def calculate_metrics_for_sims_regression(y_pred, y_true):
    """参考自: https://github.com/thuiar/Self-MM/blob/1786283c81eeb507f317fa1c70a3faf77e67cee0/utils/metricsTop.py#L73
    Args:
...
def calculate_metrics(outputs, targets, num_classes, dataset_name=None):
    if dataset_name.startswith("SIMS"):
        # CH-SIMS数据集
        return calculate_metrics_for_sims_regression(outputs, targets)
    if num_classes != 1:
        # 分类问题
