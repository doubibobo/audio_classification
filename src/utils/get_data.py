...
def get_data(args):
    ...
    for train_index, valid_index in split.split(data, label):
        print("TRAIN length: {}; VALID length: {}".format(len(train_index), len(valid_index)))
        print("TRAIN: ", train_index, "TEST:", valid_index)

        data_loader_params["slices"] = sorted(train_index)
        train_dataset = get_class_from_name(args.dataset_class)(**data_loader_params)

        data_loader_params["slices"] = sorted(valid_index)
        val_dataset = get_class_from_name(args.dataset_class)(**data_loader_params)
